...
var Plugin = function(plugin){
	var self = this;
	_.extend(self,plugin);

	Port.pop(function(port){
		self.port = port;
	})
	self.init();
}
...
Plugin.prototype = {
	...
	startSever : function(){
		...
		var child = Child.run(self.start + self.port,function(err,stdout,stderr){
			if(err || stderr){
...
exports = module.exports = {
	...
	startProxy : function(){
			var self = this;
			for (p in self.list){
				if (!self.list[p].checked){
					setTimeout(function(){self.startProxy();},1000);
					return;
				}
			}
			proxy.start(self.list);
	}
}
