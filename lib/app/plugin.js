...
Plugin.prototype = {
	checkInstalled : function(){
		...
		if(this.npm){
			...
			var check = exec("npm ls " + this.npm + " -g",function(err,stdout,stderr){
	...
	checkLatest : function(){
		...
		exec("npm view "+ self.npm, function(err,stdout,stderr){
	...
	install: function(){
		...
		var child = exec("start /B npm install -g " + this.npm, function(err,stdout,stderr){
			if(err !== null){
				self.status = null;
			}else{
				self.status = "ok";
	...
	upgrade : function(){
		...
		if (self.hasLatest){
			...
			exec("npm update -g " + this.npm, function(err,stdout,stderr){
	...
	startSever : function(){
		...
		var child = exec("start /B " + self.start,function(err,stdout,stderr){
			if(err || stderr){
	...
	openUI : function(){
		...
		if(self.status == 'started'){
			exec("start http://localhost:" + self.port + "/",function(){});
		}else{
